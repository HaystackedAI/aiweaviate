# Redshift Namespace: sds-ecommerce-0029
# Redshift Workgroup: sds-ecommerce-wg-0029
# Database: sds-ecommerce
# S3 Bucket: sds-ecommerce-redshift-0029
# Created role RedshiftS3AccessRole-0029
# Redshift IAM Role ARN: arn:aws:iam::822206589627:role/RedshiftS3AccessRole-0029
...
REDSHIFT_NAMESPACE = f'sds-ecommerce-{aws.suffix}'
REDSHIFT_WORKGROUP = f'sds-ecommerce-wg-{aws.suffix}'
REDSHIFT_DATABASE = f'sds-ecommerce'
S3_BUCKET = f'sds-ecommerce-redshift-{aws.suffix}'
...
def create_iam_role_for_redshift():
    ...
    try:
        # Get account ID
        account_id = aws.sts_client.get_caller_identity()['Account']
        
        # Create IAM role if it doesn't exist
        role_name = f'RedshiftS3AccessRole-{aws.suffix}'
        try:
            role_response = aws.iam_client.get_role(RoleName=role_name)
            print(f'Role {role_name} already exists')
            return f'arn:aws:iam::{account_id}:role/{role_name}'
        except aws.iam_client.exceptions.NoSuchEntityException:
            trust_policy = {
                "Version": "2012-10-17",
                "Statement": [
                    {
            ...
            aws.iam_client.create_role(
                RoleName=role_name,
                AssumeRolePolicyDocument=json.dumps(trust_policy)
            )
            
            # Attach necessary policies
            aws.iam_client.attach_role_policy(
                RoleName=role_name,
